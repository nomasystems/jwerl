...
sign(Data, AdditionalHeaders, Algorithm, KeyOrPem) when
    (is_map(Data) orelse is_list(Data)),
    is_map(AdditionalHeaders),
    is_atom(Algorithm),
    is_binary(KeyOrPem)
->
    encode(
        jsx:encode(Data),
        config_headers(AdditionalHeaders#{alg => algorithm_to_binary(Algorithm)}),
        KeyOrPem
    ).
...
is_string_or_uri(Value) when is_binary(Value) ->
    size(trim_left(trim_right(Value))) > 0;
is_string_or_uri(_Value) ->
    false.

trim_left(<<C, Rest/binary>>) when
    C =:= $\s orelse
        C =:= $\n orelse
        C =:= $\r orelse
        C =:= $\t
